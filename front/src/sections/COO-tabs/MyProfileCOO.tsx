...
import MyFloatingDockCustomer from "../Styles/MyFloatingDock-COO" // Keep COO specific dock
...
interface UserDetails {
  _id: string
  // Removed firstName, lastName, middleName
  email: string
  mobileNumber: string
  gender?: string
  // Removed bio
...
function MyProfile() {
  ...
  // Removed frontIdPreview, backIdPreview states
  ...
  // Removed frontIdRef, backIdRef refs

  // Refs for COO document uploads
  ...
  // State for COO document previews
  ...
  const [generalLiabilityPreview, setGeneralLiabilityPreview] = useState<string | null>(null)
  const [workersCompPreview, setWorkersCompPreview] = useState<string | null>(null)
  const [professionalIndemnityPreview, setProfessionalIndemnityPreview] = useState<string | null>(null)
  ...
  const fetchUserDetails = async () => {
    try {
      ...
      if (mappedUserDetails) {
        ...
        // Removed frontIdPreview, backIdPreview init
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    // Handle cityCoverage as an array of strings
    if (name === "cityCoverage") {
      setEditedDetails((prev) => ({
        ...prev,
        cityCoverage: value.split(",").map((item) => item.trim()),
      }))
    }
  }
  ...
  const handleEmailVerificationSuccess = useCallback(async (data: { user: UserDetails; token?: string }) => {
    ...
    const mappedUserDetails: UserDetails = {
      ...
      // COO-specific fields
      businessName: userData.businessName,
      foundedDate: userData.foundedDate ? new Date(userData.foundedDate).toISOString().split("T")[0] : undefined,
      teamSize: userData.teamSize,
      companyNumber: userData.companyNumber,
      tinNumber: userData.tinNumber,
      cityCoverage: userData.cityCoverage,
  ...
  // NEW: Handle file selection, preview, and immediate upload for COO documents
  const handleDocumentFileChange = async (
    e: React.ChangeEvent<HTMLInputElement>,
    previewSetter: React.Dispatch<React.SetStateAction<string | null>>,
    fieldName: keyof UserDetails, // Use keyof UserDetails for field name
  ) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      try {
        ...
        // Update editedDetails based on the fieldName
        setEditedDetails((prev) => ({
          ...prev,
        ...
        // Clear the field in editedDetails if upload fails
        setEditedDetails((prev) => ({
          ...prev,
          [fieldName]: null,
        }))
      ...
      // Clear the field in editedDetails if no file is selected
      setEditedDetails((prev) => ({
        ...prev,
        [fieldName]: null,
      }))
  ...
  const handleDocumentUploadClick = (element: HTMLInputElement | null) => {
  ...
  const handleSaveDetails = async (e: React.FormEvent) => {
    ...
    // Determine if COO documents were updated
    const documentsWereUpdated =
      secRegistrationPreview !== (userDetails?.secRegistration || null) ||
      businessPermitPreview !== (userDetails?.businessPermit || null) ||
      birRegistrationPreview !== (userDetails?.birRegistration || null) ||
      eccCertificatePreview !== (userDetails?.eccCertificate || null) ||
      generalLiabilityPreview !== (userDetails?.generalLiability || null) ||
      workersCompPreview !== (userDetails?.workersComp || null) ||
      professionalIndemnityPreview !== (userDetails?.professionalIndemnity || null) ||
      propertyDamagePreview !== (userDetails?.propertyDamage || null) ||
      businessInterruptionPreview !== (userDetails?.businessInterruption || null) ||
      bondingInsurancePreview !== (userDetails?.bondingInsurance || null)
    ...
    try {
      const payload = {
        ...editedDetails,
        // Removed idDocuments from payload
        secretQuestion: secretQuestion,
        secretAnswer: secretAnswer,
        secretCode: secretCode,
        // COO-specific fields
        secRegistration: secRegistrationPreview,
        businessPermit: businessPermitPreview,
        birRegistration: birRegistrationPreview,
        eccCertificate: eccCertificatePreview,
        generalLiability: generalLiabilityPreview,
        workersComp: workersCompPreview,
        professionalIndemnity: professionalIndemnityPreview,
        propertyDamage: propertyDamagePreview,
        businessInterruption: businessInterruptionPreview,
        bondingInsurance: bondingInsurancePreview,
        // Ensure foundedDate is sent as a valid date string if present
        foundedDate: editedDetails.foundedDate ? new Date(editedDetails.foundedDate).toISOString() : undefined,
        // cityCoverage is already handled by handleInputChange
      }
      ...
      // Only show observation modal if COO documents were actually changed/uploaded
      if (documentsWereUpdated) {
  ...
  const handleResetImages = async () => {
    ...
    try {
      ...
      const mappedProfilePicUserDetails: UserDetails = {
        ...
        // COO-specific fields
        businessName: profilePicUserData.businessName,
        foundedDate: profilePicUserData.foundedDate
          ? new Date(profilePicUserData.foundedDate).toISOString().split("T")[0]
          : undefined,
        teamSize: profilePicUserData.teamSize,
        companyNumber: profilePicUserData.companyNumber,
        tinNumber: profilePicUserData.tinNumber,
        cityCoverage: profilePicUserData.cityCoverage,
        aboutCompany: profilePicUserData.aboutCompany,
        secRegistration: profilePicUserData.secRegistration,
        businessPermit: profilePicUserData.businessPermit,
        birRegistration: profilePicUserData.birRegistration,
        eccCertificate: profilePicUserData.eccCertificate,
        generalLiability: profilePicUserData.generalLiability,
        workersComp: profilePicUserData.workersComp,
        professionalIndemnity: profilePicUserData.professionalIndemnity,
        propertyDamage: profilePicUserData.propertyDamage,
        businessInterruption: profilePicUserData.businessInterruption,
        bondingInsurance: profilePicUserData.bondingInsurance,
      }
      ...
      const mappedCoverPhotoUserDetails: UserDetails = {
        ...
        // COO-specific fields
        businessName: coverPhotoUserData.businessName,
        foundedDate: coverPhotoUserData.foundedDate
          ? new Date(coverPhotoUserData.foundedDate).toISOString().split("T")[0]
          : undefined,
        teamSize: coverPhotoUserData.teamSize,
        companyNumber: coverPhotoUserData.companyNumber,
        tinNumber: coverPhotoUserData.tinNumber,
        cityCoverage: coverPhotoUserData.cityCoverage,
        aboutCompany: coverPhotoUserData.aboutCompany,
        secRegistration: coverPhotoUserData.secRegistration,
        businessPermit: coverPhotoUserData.businessPermit,
        birRegistration: coverPhotoUserData.birRegistration,
        eccCertificate: coverPhotoUserData.eccCertificate,
        generalLiability: coverPhotoUserData.generalLiability,
        workersComp: coverPhotoUserData.workersComp,
        professionalIndemnity: coverPhotoUserData.professionalIndemnity,
        propertyDamage: coverPhotoUserData.propertyDamage,
        businessInterruption: coverPhotoUserData.businessInterruption,
        bondingInsurance: coverPhotoUserData.bondingInsurance,
      }
            ...
            {isEditingDetails ? (
              <form onSubmit={handleSaveDetails} className="grid grid-cols-1 gap-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Business Name (replaces First/Middle/Last Name) */}
                ...
                <div className="md:col-span-full">
                  ...
                  <div className="relative">
                    ...
                    <textarea
                      id="aboutCompany"
                      name="aboutCompany"
                      value={editedDetails.aboutCompany || ""}
                      onChange={handleInputChange}
                      rows={4}
                      className="w-full rounded-md border border-gray-300 py-2 pl-10 pr-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-500"
                    />
                ...
                <div className="md:col-span-full">
                  <h3 className="text-xl font-medium text-gray-700 mb-6 mt-8">Company Details</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Founded Date */}
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          id="foundedDate"
                          name="foundedDate"
                          value={editedDetails.foundedDate || ""}
                          onChange={handleInputChange}
                          className="w-full rounded-md border border-gray-300 py-2 pl-10 pr-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-500"
                        />
                      </div>
                    ...
                    {/* City Coverage */}
                    <div>
                      <label htmlFor="cityCoverage" className="block text-sm font-medium text-gray-700 mb-1">
                        City Coverage (comma-separated)
                      </label>
                      <div className="relative">
                        ...
                        <input
                          ...
                          id="cityCoverage"
                          name="cityCoverage"
                          value={editedDetails.cityCoverage?.join(", ") || ""}
                          onChange={handleInputChange}
                          className="w-full rounded-md border border-gray-300 py-2 pl-10 pr-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-500"
                ...
                {/* COO Document Uploads */}
                <div className="md:col-span-full mt-8">
                  <h4 className="text-lg font-medium text-gray-700 mb-4">Business Documents & Insurance</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {/* SEC Registration */}
                    <div>
                      <label htmlFor="secRegistration" className="mb-2 block text-sm font-medium">
                    ...
                    {/* Business Permit */}
                    <div>
                      <label htmlFor="businessPermit" className="mb-2 block text-sm font-medium">
                    ...
                    {/* BIR Registration */}
                    <div>
                      <label htmlFor="birRegistration" className="mb-2 block text-sm font-medium">
                    ...
                    {/* ECC Certificate */}
                    <div>
                      <label htmlFor="eccCertificate" className="mb-2 block text-sm font-medium">
                      ...
                      >
                        {eccCertificatePreview ? (
                          ...
                          <>
                            <Upload className="h-8 w-8 text-gray-400 mb-2" />
                            <p className="text-sm text-gray-500">Upload ECC Certificate</p>
                            <p className="text-xs text-gray-400 mt-1">JPG, PNG or PDF up to 5MB</p>
                          </>
                        )}
                        <input
                    ...
                    {/* General Liability */}
                    <div>
                      <label htmlFor="generalLiability" className="mb-2 block text-sm font-medium">
                      ...
                      >
                        {generalLiabilityPreview ? (
                          ...
                          <>
                            <Upload className="h-8 w-8 text-gray-400 mb-2" />
                            <p className="text-sm text-gray-500">Upload General Liability</p>
                            <p className="text-xs text-gray-400 mt-1">JPG, PNG or PDF up to 5MB</p>
                          </>
                        )}
                        <input
                    ...
                    {/* Workers Comp */}
                    <div>
                      <label htmlFor="workersComp" className="mb-2 block text-sm font-medium">
                      ...
                      >
                        {workersCompPreview ? (
                          ...
                          <>
                            <Upload className="h-8 w-8 text-gray-400 mb-2" />
                            <p className="text-sm text-gray-500">Upload Workers Comp</p>
                            <p className="text-xs text-gray-400 mt-1">JPG, PNG or PDF up to 5MB</p>
                          </>
                        )}
                        <input
                    ...
                    <div>
                      <label htmlFor="professionalIndemnity" className="mb-2 block text-sm font-medium">
                      ...
                      >
                        {professionalIndemnityPreview ? (
                          ...
                          <>
                            <Upload className="h-8 w-8 text-gray-400 mb-2" />
                            <p className="text-sm text-gray-500">Upload Professional Indemnity</p>
                            <p className="text-xs text-gray-400 mt-1">JPG, PNG or PDF up to 5MB</p>
                          </>
                        )}
                        <input
                    ...
                    {/* Property Damage */}
                    <div>
                      <label htmlFor="propertyDamage" className="mb-2 block text-sm font-medium">
                      ...
                      >
                        {propertyDamagePreview ? (
                          ...
                          <>
                            <Upload className="h-8 w-8 text-gray-400 mb-2" />
                            <p className="text-sm text-gray-500">Upload Property Damage</p>
                            <p className="text-xs text-gray-400 mt-1">JPG, PNG or PDF up to 5MB</p>
                          </>
                        )}
                        <input
                    ...
                    <div>
                      <label htmlFor="businessInterruption" className="mb-2 block text-sm font-medium">
                      ...
                      >
                        {businessInterruptionPreview ? (
                          ...
                          <>
                            <Upload className="h-8 w-8 text-gray-400 mb-2" />
                            <p className="text-sm text-gray-500">Upload Business Interruption</p>
                            <p className="text-xs text-gray-400 mt-1">JPG, PNG or PDF up to 5MB</p>
                          </>
                        )}
                        <input
                    ...
                    {/* Bonding Insurance */}
                    <div>
                      <label htmlFor="bondingInsurance" className="mb-2 block text-sm font-medium">
                      ...
                      >
                        {bondingInsurancePreview ? (
                          ...
                          <>
                            <Upload className="h-8 w-8 text-gray-400 mb-2" />
                            <p className="text-sm text-gray-500">Upload Bonding Insurance</p>
                            <p className="text-xs text-gray-400 mt-1">JPG, PNG or PDF up to 5MB</p>
                          </>
                        )}
                        <input
                    ...
                    onClick={() => {
                      ...
                      if (userDetails) {
                        setEditedDetails({
                          ...
                          foundedDate: userDetails.foundedDate,
                          teamSize: userDetails.teamSize,
                          companyNumber: userDetails.companyNumber,
                          tinNumber: userDetails.tinNumber,
                          cityCoverage: userDetails.cityCoverage,
                          aboutCompany: userDetails.aboutCompany,
                          secRegistration: userDetails.secRegistration,
                          businessPermit: userDetails.businessPermit,
                          birRegistration: userDetails.birRegistration,
                          eccCertificate: userDetails.eccCertificate,
                          generalLiability: userDetails.generalLiability,
                          workersComp: userDetails.workersComp,
                          professionalIndemnity: userDetails.professionalIndemnity,
                          propertyDamage: userDetails.propertyDamage,
                          businessInterruption: userDetails.businessInterruption,
                          bondingInsurance: userDetails.bondingInsurance,
                        })
                        setSelectedLocation(userDetails.location ? { ...userDetails.location } : null)
                        setOriginalEmail(userDetails.email)
                        // Removed frontIdPreview, backIdPreview reset
                        ...
                        // Reset COO document previews
                        setSecRegistrationPreview(userDetails.secRegistration || null)
                        setBusinessPermitPreview(userDetails.businessPermit || null)
                        setBirRegistrationPreview(userDetails.birRegistration || null)
                        setEccCertificatePreview(userDetails.eccCertificate || null)
                        setGeneralLiabilityPreview(userDetails.generalLiability || null)
                        setWorkersCompPreview(userDetails.workersComp || null)
                        setProfessionalIndemnityPreview(userDetails.professionalIndemnity || null)
                        setPropertyDamagePreview(userDetails.propertyDamage || null)
                        setBusinessInterruptionPreview(userDetails.businessInterruption || null)
                        setBondingInsurancePreview(userDetails.bondingInsurance || null)
                      }
                      setSaveDetailsError(null)
                      setEmailError(null)
                      setEmailValidationStatus("idle")
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Business Name (replaces Full Name) */}
                <div className="md:col-span-2 rounded-lg border border-gray-200 bg-gray-50 p-4">
                  <h4 className="mb-1 flex items-center text-sm font-medium text-gray-500">
                    <Building className="mr-2 h-4 w-4" /> Business Name
                  </h4>
                  <p className="text-gray-900">{userDetails?.businessName || "N/A"}</p>
                </div>
                ...
                <div className="md:col-span-2 rounded-lg border border-gray-200 bg-gray-50 p-4">
                  <h4 className="mb-1 flex items-center text-sm font-medium text-gray-500">
                    <Text className="mr-2 h-4 w-4" /> About Company
                  </h4>
                  <p className="text-gray-900">{userDetails?.aboutCompany || "No description provided"}</p>
                </div>
                ...
                {/* COO-specific fields for display */}
                <div className="md:col-span-2 mt-8">
                  <h3 className="text-xl font-medium text-gray-700 mb-6">Company Details</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                      <h4 className="mb-1 flex items-center text-sm font-medium text-gray-500">
                        <CalendarDays className="mr-2 h-4 w-4" /> Founded Date
                      </h4>
                      <p className="text-gray-900">
                        {userDetails?.foundedDate ? new Date(userDetails.foundedDate).toLocaleDateString() : "N/A"}
                      </p>
                    </div>
                    <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                      <h4 className="mb-1 flex items-center text-sm font-medium text-gray-500">
                        <Users className="mr-2 h-4 w-4" /> Team Size
                      </h4>
                      <p className="text-gray-900">{userDetails?.teamSize || "N/A"}</p>
                    </div>
                    <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                      <h4 className="mb-1 flex items-center text-sm font-medium text-gray-500">
                        <FileText className="mr-2 h-4 w-4" /> Company Number
                      </h4>
                      <p className="text-gray-900">{userDetails?.companyNumber || "N/A"}</p>
                    </div>
                    <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                      <h4 className="mb-1 flex items-center text-sm font-medium text-gray-500">
                        <FileText className="mr-2 h-4 w-4" /> TIN Number
                      </h4>
                      <p className="text-gray-900">{userDetails?.tinNumber || "N/A"}</p>
                    </div>
                    <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                      <h4 className="mb-1 flex items-center text-sm font-medium text-gray-500">
                        <MapPin className="mr-2 h-4 w-4" /> City Coverage
                      </h4>
                      <p className="text-gray-900">
                        {userDetails?.cityCoverage?.length ? userDetails.cityCoverage.join(", ") : "N/A"}
                      </p>
                    </div>
                  </div>
                </div>

                {/* COO Documents Display */}
                <div className="md:col-span-2 mt-8">
                  <h4 className="text-xl font-medium text-gray-700 mb-4">Business Documents & Insurance</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {/* SEC Registration */}
                    <div>
                      <p className="text-gray-700 text-sm mb-1">SEC Registration:</p>
                      {userDetails?.secRegistration ? (
                        <div
                          className="relative w-full h-32 cursor-pointer"
                          onClick={() => handleImageClick(userDetails.secRegistration || null)}
                        >
                          <img
                            src={userDetails.secRegistration || "/placeholder.svg"}
                    ...
                    {/* Business Permit */}
                    <div>
                      <p className="text-gray-700 text-sm mb-1">Business Permit:</p>
                      {userDetails?.businessPermit ? (
                        <div
                          className="relative w-full h-32 cursor-pointer"
                          onClick={() => handleImageClick(userDetails.businessPermit || null)}
                        >
                          <img
                            src={userDetails.businessPermit || "/placeholder.svg"}
                            alt="Business Permit"
                            className="w-full h-32 object-cover rounded-md border border-gray-300 filter blur-sm hover:blur-[2px] transition-all"
                          />
                          <div className="absolute inset-0 flex items-center justify-center">
                    ...
                    {/* BIR Registration */}
                    <div>
                      <p className="text-gray-700 text-sm mb-1">BIR Registration:</p>
                      {userDetails?.birRegistration ? (
                        <div
                          className="relative w-full h-32 cursor-pointer"
                          onClick={() => handleImageClick(userDetails.birRegistration || null)}
                        >
                          <img
                            src={userDetails.birRegistration || "/placeholder.svg"}
                    ...
                    {/* ECC Certificate */}
                    <div>
                      <p className="text-gray-700 text-sm mb-1">ECC Certificate:</p>
                      {userDetails?.eccCertificate ? (
                        <div
                          className="relative w-full h-32 cursor-pointer"
                          onClick={() => handleImageClick(userDetails.eccCertificate || null)}
                        >
                          <img
                            src={userDetails.eccCertificate || "/placeholder.svg"}
                            alt="ECC Certificate"
                            className="w-full h-32 object-cover rounded-md border border-gray-300 filter blur-sm hover:blur-[2px] transition-all"
                          />
                          <div className="absolute inset-0 flex items-center justify-center">
                    ...
                    {/* General Liability */}
                    <div>
                      <p className="text-gray-700 text-sm mb-1">General Liability:</p>
                      {userDetails?.generalLiability ? (
                        <div
                          className="relative w-full h-32 cursor-pointer"
                          onClick={() => handleImageClick(userDetails.generalLiability || null)}
                        >
                          <img
                            src={userDetails.generalLiability || "/placeholder.svg"}
                    ...
                    {/* Workers Comp */}
                    <div>
                      <p className="text-gray-700 text-sm mb-1">Workers Comp:</p>
                      {userDetails?.workersComp ? (
                        <div
                          className="relative w-full h-32 cursor-pointer"
                          onClick={() => handleImageClick(userDetails.workersComp || null)}
                        >
                          <img
                            src={userDetails.workersComp || "/placeholder.svg"}
                            alt="Workers Comp"
                            className="w-full h-32 object-cover rounded-md border border-gray-300 filter blur-sm hover:blur-[2px] transition-all"
                          />
                          <div className="absolute inset-0 flex items-center justify-center">
                    ...
                    {/* Professional Indemnity */}
                    <div>
                      <p className="text-gray-700 text-sm mb-1">Professional Indemnity:</p>
                      {userDetails?.professionalIndemnity ? (
                        <div
                          className="relative w-full h-32 cursor-pointer"
                          onClick={() => handleImageClick(userDetails.professionalIndemnity || null)}
                        >
                          <img
                            src={userDetails.professionalIndemnity || "/placeholder.svg"}
                    ...
                    {/* Property Damage */}
                    <div>
                      <p className="text-gray-700 text-sm mb-1">Property Damage:</p>
                      {userDetails?.propertyDamage ? (
                        <div
                          className="relative w-full h-32 cursor-pointer"
                          onClick={() => handleImageClick(userDetails.propertyDamage || null)}
                        >
                          <img
                            src={userDetails.propertyDamage || "/placeholder.svg"}
                            alt="Property Damage"
                            className="w-full h-32 object-cover rounded-md border border-gray-300 filter blur-sm hover:blur-[2px] transition-all"
                          />
                          <div className="absolute inset-0 flex items-center justify-center">
                    ...
                    {/* Business Interruption */}
                    <div>
                      <p className="text-gray-700 text-sm mb-1">Business Interruption:</p>
                      {userDetails?.businessInterruption ? (
                        <div
                          className="relative w-full h-32 cursor-pointer"
                          onClick={() => handleImageClick(userDetails.businessInterruption || null)}
                        >
                          <img
                            src={userDetails.businessInterruption || "/placeholder.svg"}
                    ...
                    {/* Bonding Insurance */}
                    <div>
                      <p className="text-gray-700 text-sm mb-1">Bonding Insurance:</p>
                      {userDetails?.bondingInsurance ? (
                        <div
                          className="relative w-full h-32 cursor-pointer"
                          onClick={() => handleImageClick(userDetails.bondingInsurance || null)}
                        >
                          <img
                            src={userDetails.bondingInsurance || "/placeholder.svg"}
                            alt="Bonding Insurance"
                            className="w-full h-32 object-cover rounded-md border border-gray-300 filter blur-sm hover:blur-[2px] transition-all"
                          />
                          <div className="absolute inset-0 flex items-center justify-center">
                            <span className="bg-black/50 text-white px-3 py-1 rounded-full text-sm">Click to view</span>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="relative px-4 pb-8">
          ...
          <div className="pt-20">
            <div className="flex items-center gap-2 justify-between">
              <div className="flex items-center gap-2">
                <h1 className="text-2xl font-medium text-gray-700">{userDetails?.businessName || "Company"}</h1>
            ...
            <div className="flex items-center gap-2 mt-8 text-gray-600">
              <span>{userDetails?.aboutCompany}</span>
            </div>
          </div>
      ...
      {/* Warning Modal for Document Upload */}
      {showWarningModal && (
        <div
